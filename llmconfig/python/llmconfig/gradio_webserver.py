...
def preset_params(template_name, temperature, best_of, presence_penalty, 
                  frequency_penalty, repetition_penalty, top_p, top_k, length_penalty):
    try:
        params_load = {
        ...
        gr.Info(f"set template params fail, info is {e}")
        return
    path_param = {"templateName": template_name}
    response = requests.post(f"http://127.0.0.1:8001/templates/params", json=pload, params=path_param)
    json_data = json.loads(response.text)
    if response.status_code == 200:
        gr.Info("preset template parameters success")
    else:
        gr.Info(f"preset parameters failed, info is {json_data}")

def reset_params():
    # 返回所有滑块控件的默认值
    return [
        0.0,  # temperature.value
        0,    # best_of.value
        0.0,  # presence_penalty.value
        0.0,  # frequency_penalty.value
        1.0,  # repetition_penalty.value
        1.0,  # top_p.value
        -1,   # top_k.value
        1.0   # length_penalty.value
    ]
...
def bot(history, no_stream_chat, prompt_header="You are a helpful assistant.", prompt_template="",
        maxContentRound: int = 0, maxLength: int = 800, temperature: float = 0.0, best_of: int = 0,
...
with gr.Blocks() as demo:
    ...
    with gr.Row(equal_height=True):  # Ensure equal height for left and right columns
        with gr.Column():
            with gr.Row():
        ...
        with gr.Column():
            ...
            with gr.Row():
                ...
                best_of = gr.Slider(minimum=1, maximum=10, step=1, value=1, label="best_of")
            with gr.Row():
            ...
            with gr.Row():     
                btn_preset=gr.Button(value="下发预设参数")
                btn_reset=gr.Button(value="重置预设参数")
                btn_preset.click(preset_params, inputs=[dropdown, temperature, best_of, presence_penalty, frequency_penalty, 
                                                        repetition_penalty, top_p, top_k, length_penalty])
                btn_reset.click(fn=reset_params,
                                inputs=None,  # 重置函数不需要输入
                                outputs=[temperature, best_of, presence_penalty, frequency_penalty, 
                                repetition_penalty, top_p, top_k, length_penalty]   
        )
    txt_msg = txt.submit(add_text, [chatbot, txt], [chatbot, txt], queue=False).then(
        bot, inputs=[chatbot, no_stream_chat, prompt_header, current_prompt_template, max_round, length,
                     temperature, best_of,
...
if __name__ == "__main__":
    ...
    demo.queue(default_concurrency_limit=100).launch(server_name=args.host,
