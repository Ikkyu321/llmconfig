...
def add_template(template_name, template_content):
    ...
    response = requests.post(f"http://127.0.0.1:8001/prompt/templates", params=path_param, json=pload)
    json_data = json.loads(response.text)
    if json_data["code"] == "200":
...
def bot(history, no_stream_chat, prompt_header="You are a helpful assistant.", prompt_template="",
        ...
        use_beam_search: bool = False, length_penalty: float = 0.0
        ):
    print("\n\n")
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            with gr.Row():
                use_beam_search = gr.Checkbox(label="use_beam_search", elem_classes=["display:flex",
