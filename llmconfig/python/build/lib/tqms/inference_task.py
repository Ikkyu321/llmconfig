import base64
from tqms.logger import init_logger
import types
...
class LLMTask(InferenceTask):
    ...
    def post_process(self, predicts):
        # if not (isinstance(predicts, dict) or isinstance(predicts, types.GeneratorType)):
        #     raise RuntimeError(f"only dict or generator allowed, but got {type(predicts).__name__}")
        ...
        if isinstance(predicts, dict):
            if predicts.get("output", None) is not None and len(predicts["output"]) == 0:
                ...
                logging.info(f"this is a choice predict")
