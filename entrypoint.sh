...
if [ "$ENABLE_LLMCONFIG" -eq 1 ]; then
    echo "ENABLE_LLMCONFIG is enabled. Starting llmconfig..."
    cd /workspace/app/llmconfig/python/llmconfig 
    sed -i 's/"taskType":[^,]*/"taskType":'"\"$TASK_TYPE\""'/' meta.json
    #sed -i 's/"llmInferenceUrl":[^,]*/"llmInferenceUrl":'"\"$LLM_INFERENCE_URL\""'/' meta.json
    cat meta.json
    nohup python3 gradio_webserver.py > /workspace/app/logs/gradio.log 2>&1 &
    nohup python3 llmconfigcli.py > /workspace/app/logs/llmconfig.log 2>&1 &
else
    echo "ENABLE_LLMCONFIG is not enabled. Skipping llmconfig startup."
fi
...
if [ -n "$EMBEDDING_OFFLINE" ]; then
    ...
    #> /workspace/app/logs/vllm.log 2>&1 &
